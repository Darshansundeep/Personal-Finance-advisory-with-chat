...
const chatSystemPrompt = `You are a senior financial advisor. Provide only brief, actionable advice:
...
export default function Advisory() {
  ...
  const getFinancialAdvice = async () => {
    try {
      ...
      setAdvisoryContent(formattedContent);
    } catch (error) {
      console.error('Error fetching advice:', error);
      setError('Unable to generate financial advice. Please try again later.');
    } finally {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    setMessages([
      {
        role: 'assistant',
        content: `Hello! I'm your financial expert. How can I help you today?`,
      },
    ]);
  };
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            {
              role: 'assistant',
              content: `Here is the user's financial context: ${JSON.stringify({
                personal: data.personal,
                income: data.income,
                expenses: data.expenses,
                loans: data.loans,
      ...
      setMessages([
        ...newMessages,
        { role: 'assistant', content: 'I apologize, but I encountered an error. Please try asking your question again.' },
      ]);
    }
  };
